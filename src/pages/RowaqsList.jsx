...
function RowaqsList() {
  const { rowaqs, deleteRowaq } = useAppData();
    ...
    <div className="management-page">
      ...
      <div className="table-controls" style={{ justifyContent: 'space-between', flexDirection: 'row-reverse', marginBottom: '15px' }}>
        <div className="action-buttons" style={{ flexDirection: 'row-reverse' }}>
          <Link to="/riwaqs/create" className="btn btn-primary" style={{ textDecoration: 'none', background: 'var(--accent-gold)', color: 'var(--bg-black)' }}>
            <Plus size={16} /> إضافة رواق
          </Link>
        </div>
        <div className="table-stats">النتائج ({rowaqs.length})</div>
      </div>
              ...
              rowaqs.map(r => (
                <tr key={r.id}>
                  ...
                  <td className="actions-cell">
                    ...
                    <Link to={`/riwaqs/create?id=${r.id}`} className="action-icon edit" style={{textDecoration: 'none', color: 'inherit'}}><Edit size={16}/></Link>
                    <button className="action-icon delete" onClick={() => deleteRowaq(r.id)}><Trash2 size={16}/></button>
                  </td>
                </tr>
              ))
            )}
